...
public class NotebookMain {
    public static void main(String[] args) {
        // Создаём множество экземпляров класса Notebook
        Notebook ntb1 = new Notebook("Acer", "Aspire 3 A315-44P-R7ZT", 1.78, 15.6, "AMD Ryzen 5", 8, 512, "без ОС",
                "серебряный");
        Notebook ntb2 = new Notebook("Lenovo", "ThinkPad X280", 1.13, 12.5, "AIntel Core i5 8250U", 8, 256,
                "Windows 10 Pro",
                "черный");
        Notebook ntb3 = new Notebook("Xiaomi", "Book Pro 16", 1.8, 16, "Intel Core i5 1240p", 16, 512,
                "Windows 11 Home",
                "черный");
        Notebook ntb4 = new Notebook("ASUS", "Vivobook Go E1504FA-BQ585", 1.63, 15.6, "AMD Ryzen 3 7320U", 8, 256,
                "без ОС",
                "черный");

        Set<Notebook> ntbks = new HashSet<>(Arrays.asList(ntb1, ntb2, ntb3, ntb4));

        // Запрос условий фильтрации
        Map<Integer, String> fltrs = getFilters();
        // Поиск с учетом фиьтров
        Set<Notebook> ntbksFiltered = search(ntbks, fltrs);

        // Вывод результата
        if (ntbksFiltered.isEmpty())
            System.out.println("\nПо данным параметрам >" + fltrs + "< ничего не найдено");
        else {
    ...
    public static Set<Notebook> search(Set<Notebook> ntbks, Map<Integer, String> fltrs) {
        ...
        // Фильтруем и добавляем в ntbksFiltered
        for (Notebook notebook : ntbks) {
            if (notebook.getWeight() >= fltrsWeigth && notebook.getScreenDiag() >= fltrsScreen &&
                ...
                if (fltrs.containsKey(5)) {
                    if (notebook.getColor().equals(fltrs.get(5).toUpperCase()))
                        ntbksFiltered.add(notebook);
                } else
                    ntbksFiltered.add(notebook);
            }
        }
    ...
    public static Map<Integer, String> getFilters() {
        ...
        while (flag) {
            ...
            if (menu.containsKey(filterKey)) {
                ...
                System.out.println("Неверное значение категории!!!");
